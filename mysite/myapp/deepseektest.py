...
response = client.chat.completions.create(
    ...
    messages=[
        ...
        """},
       # {"role": "system", "content": "당신은 협업중인 동료 백엔드 개발자입니다. 당신은 클린코드 원칙을 항상 최우선적으로 생각하며 코드리뷰를 진행합니다. 코드리뷰를 진행할 때는 꼼꼼히 분석해서 클린코드 원칙을 지켰는가를 우선적으로 판단하고 그 다음 그 외의 문제가 있는지 파악합니다"},
       # {"role": "system", "content": "당신은 협업중인 동료 백엔드 개발자입니다. 당신은 성능과 최적화를 항상 최우선적으로 생각하며 코드리뷰를 진행합니다. 코드리뷰를 진행할 때는 꼼꼼히 분석해서 성능과 최적화 적인 부분에서 잘 짜여졌는지 우선적으로 판단하고 그 다음 그 외의 문제가 있는지 파악합니다"},
        {"role": "user", "content": '''
